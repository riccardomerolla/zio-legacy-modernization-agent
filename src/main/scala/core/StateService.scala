...
object StateService:
  ...
  def live(stateDir: Path): ZLayer[FileService, Nothing, StateService] = ZLayer.fromFunction { (fileService: FileService) =>
    new StateService {
      ...
      override def createCheckpoint(step: String): ZIO[Any, Throwable, Unit] =
        for
          _ <- ZIO.logInfo(s"Creating checkpoint for step: $step")
